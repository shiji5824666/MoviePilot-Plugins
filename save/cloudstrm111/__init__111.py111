...
class CloudStrm(_PluginBase):
    ...
    _dav_pass = None
    _observer = []
    _video_formats = ('.mp4', '.avi', '.rmvb', '.wmv', '.mov', '.mkv', '.flv', '.ts', '.webm', '.iso', '.mpg', '.m2ts', '.mpeg', '.vob')
    _ass_formats = ('.ass', '.ssa', '.srt', '.jpg', '.nfo', '.idx', '.sub')
                    ...
                    if Path(dest_file).is_dir():
                        ...
                        if dest_file.lower().endswith(self._video_formats):
                            ...
                            if self._copy_files and not self._alist_webdav:
                                ...
                                if self._copy_files and self._alist_webdav and dest_file.lower().endswith(self._ass_formats):
                                    p=1
                                    while p<10:
                                            try:
